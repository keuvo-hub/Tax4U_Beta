...
const AttendanceSetting = require('../models/attendance_setting.model')
const { employeeElement } = require('./users')

// Attendance settings
// post AttendanceSetting
exports.postAttendanceSetting = async (req, res, next) => {
    try {
        const { body } = req;
        if (body._id) {
...
exports.getAttendanceSetting = async (req, res, next) => {
    try {
...
exports.delAttendanceSetting = async (req, res, next) => {
    try {
        const { query } = req;
        ...
        const attendance = await Attendance?.aggregatePaginate(Attendance.aggregate([
            ...
            { $unwind: { path: '$employee_key', preserveNullAndEmptyArrays: true } },
            {
                $project: {
                    ...
                    employee: {$concat: ["$employee_key.firstName"," ","$employee_key.lastName","-","$employee_key.username"]},
...
// delete Attendance
exports.delAttendance = async (req, res, next) => {
    try {
        const { query } = req;
...
// get Attendance
exports.getEmployeePlusLastPunch = async (req, res, next) => {
    try {
        const { query } = req;
        let filter = {};
        ...
        const attendance = await User?.aggregatePaginate(User.aggregate([
            ...
            {
                $project: {
                    employee:  {$concat: ["$firstName"," ","$firstName"]},
...
exports.getEmployeePunchInOut = async (req, res, next) => {
    try {
        const { query } = req;
        ...
        const punchIn = await Attendance.aggregate([
            ...
            { $unwind: { path: '$employee', preserveNullAndEmptyArrays: true } },
            {
                $project: {
                    _id: "$employee._id",
                    employee: {$concat: ["$employee.firstName"," ","$employee.firstName"]},
                    start_time: { $ifNull: ["$start_time", 0] },
                    end_time: { $ifNull: ["$end_time", 0] },
    ...
    try {
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                name: userExit?.middle_name ? `${userExit?.first_name} ${userExit?.middle_name} ${userExit?.last_name}` : `${userExit?.firstName} ${userExit?.lastName}`
            },
        })
...
exports.getOneClockInOut = async (req, res, next) => {
    try {
...
// breakTime start
exports.breakTimeStart = async (req, res, next) => {
    try {
        const { query } = req;
        const userExit = await User.findOne({ key: req.query.key });
        if (!userExit) {
