...
const finalEmailToUser = async (data = {}) => {
    ...
    const info = await transporter.sendMail({
        from: email_from,
        to: `${data.user.email}`,
        subject: `Final confirmation email`,
        html:
            `
                    ...
                    <div style="margin: auto; width: 50%; padding-top: 50px;">
                        <div
                            ...
                            <div style="border-radius: 5px; background-color: rgba(0,0,0, .1); padding: 10px; margin: 30px 0px;">
                                <h4>
                                Great News! Your tax files have been completed successfully. To download the files please click <a href="${envFileData?.website_domain_name}">${capitalizeFirstLetter(envFileData?.website_name)}</a>. Thanks for staying with us. Please contact us if you have any problems. 
                                </h4>
                            </div>
...
module.exports = finalEmailToUser;